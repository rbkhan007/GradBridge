...
export async function POST(req: Request) {
  ...
  if (message.length > 8000) return NextResponse.json({ error: "message too long (max 8000 chars)" }, { status: 413 });
  ...
  if (keyRow) {
    ...
    if (freeUsed >= 5) {
      return NextResponse.json({ error: "Daily free message limit reached (5/5). Add your own API key in Settings for unlimited usage." }, { status: 429 });
    }
  }
